...
class ExampleCubes: AppI {
    ...
    func update() -> Bool {
        if !processEvents(&width, height: &height, debug: &debug, reset: &reset) {
            ...
            let view = lookAt(eye: [0.0, 0.0, -35.0], center: [0.0, 0.0, 0.0], up: [0.0, 1.0, 0.0])
            let proj = perspective_fov(60.0, width: Float(width), height: Float(height), near: 0.1, far: 100.0)
            ...
            for yy in 0...10 {
                for xx in 0...10 {
                    var mtx = rotateXY(time + Float(xx)*0.21, ay: time + Float(yy)*0.37)
                    mtx[3][0] = -15.0 + Float(xx)*3.0
                    mtx[3][1] = -15.0 + Float(yy)*3.0
                    mtx[3][2] = 0.0
