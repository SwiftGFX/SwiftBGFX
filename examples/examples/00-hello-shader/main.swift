...
class HelloWorld: AppI {
    ...
    func update() -> Bool {
        if !processEvents(&width, height: &height, debug: &debug, reset: &reset) {
            ...
            bgfx.debugTextClear()
            bgfx.debugTextPrint(x: 0, y: 1, foreColor: .white, backColor: .blue, format: "bgfx/examples/00-hello-shader")
            bgfx.debugTextPrint(x: 0, y: 2, foreColor: .white, backColor: .blue, format: "%08.4lf secs", timer.counter)
