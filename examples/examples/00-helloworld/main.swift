...
class HelloWorld: AppI {
    ...
    func update() -> Bool {
        if !processEvents(&width, height: &height, debug: &debug, reset: &reset) {
            ...
            bgfx.debugTextPrint(x: 0, y: 1, foreColor: .white, backColor: .blue, format: "bgfx/examples/00-helloworld")
            bgfx.debugTextPrint(x: 0, y: 2, foreColor: .white, backColor: .cyan, format: "Description: Initialization and debug text.")
