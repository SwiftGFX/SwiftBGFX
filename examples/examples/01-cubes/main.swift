...
class ExampleCubes: AppI {
    ...
    func update() -> Bool {
        if !processEvents(&width, height: &height, debug: &debug, reset: &reset) {
            ...
            bgfx.debugTextClear()
            bgfx.debugTextPrint(x: 0, y: 1, foreColor: .white, backColor: .blue, format: "bgfx/examples/01-cubes")
            bgfx.debugTextPrint(x: 0, y: 2, foreColor: .white, backColor: .cyan, format: "Description: Rendering simple static mesh.")
